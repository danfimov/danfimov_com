...
const baseConfig = {
  ...
  sizes: ["(min-width: 1760px) 828px", "(min-width: 1024) calc(75vw - 20px - 2 * 42px)", "calc(100vw - 2 * 20px)"].join(
    ", ",
  ),
  formats: ["avif", "webp"],
...
export default function (window, content, outputPath) {
  ...
  return Promise.all(images.map((image) => buildImage(image, imagesSourcePath, imagesOutputPrefix, window)));
}
